...
- profile_set: 设置基本信息（Bot名字、主人名字等核心身份信息）
- profile_get: 获取基本信息
...
6. 当用户告诉你他的名字、称呼，或者给你起名字、设定人设时，使用 profile_set 保存
7. 不确定时主动使用工具查找，而非凭猜测回答`;
...
**重要：基本信息的特殊处理**
如果用户告诉了他的名字或称呼，请使用路径 ["基本信息", "主人", "名字"]
如果用户给Bot起了名字，请使用路径 ["基本信息", "Bot", "名字"]
如果用户描述了Bot的人设/性格，请使用路径 ["基本信息", "Bot", "人设"]
如果用户描述了与Bot的关系，请使用路径 ["基本信息", "关系", "描述"]

以JSON数组格式返回，每个元素包含path（路径数组）和content（内容字符串）：
[
  { "path": ["个人信息", "姓名"], "content": "小魏" },
  { "path": ["工作", "公司"], "content": "杭州智诺" },
  { "path": ["基本信息", "主人", "名字"], "content": "小魏" }
]
